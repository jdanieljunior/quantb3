...
def run_tuesday_job(exec_date: date = None, seed: int = 42) -> dict:
    ...
    try:
        ...
        # 4. Carregar posições e equity atuais
        equity_df = get_equity_curve()
        if not equity_df.empty:
            last_equity = equity_df.iloc[-1]
            cash = float(last_equity["cash"])
            pos_value = float(last_equity["pos_value"])
            equity = float(last_equity["equity"])
        else:
            cash = CAPITAL
            pos_value = 0.0
            equity = CAPITAL

        # Reconstrói posições atuais (simplificado — usa sinais anteriores)
        from src.db.repositories import get_current_positions
        current_pos_list = get_current_positions()
        positions: Dict[str, Dict] = {
            p["ticker"]: {
                "qty": p["qty"],
                "avg_price": p["avg_price"],
                "stop": p.get("stop_price"),
                "take": p.get("take_price"),
            }
